...
const SortableImageItem = ({ 
  ...
  isFeatured: boolean;
  onEditMetadata: (id: string, metadata: { altText: string; caption: string }) => void;
  ...
  return (
    <div
      ...
      className={`border rounded-lg overflow-hidden bg-gray-800 transition-all ${
        image.isDeleted ? 'opacity-50 border-red-500' : 
      ...
      <div className="relative">
        ...
        {image.isDeleted && (
          <div className="absolute inset-0 bg-red-500/20 flex items-center justify-center">
            <span className="text-red-500 font-bold text-lg">سيتم حذف هذه الصورة</span>
          </div>
        )}
...
const ImageUploader: React.FC<ImageUploaderProps> = ({
  ...
  const editImageMetadata = (id: string, metadata: { altText: string; caption: string }) => {
    const updatedImages = images.map(image => {
      if (image.id === id) {
        return {
          ...image,
  ...
  const deleteSelectedImages = () => {
    ...
    updatedImages = updatedImages.filter(image => 
      !(selectedImages.includes(image.id) && !image.isExisting)
    );
