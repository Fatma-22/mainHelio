...
import { getPropertyRequests, approvePropertyRequest, rejectPropertyRequest, editAndPublishProperty } from './services/propertyRequestService';
...
const App: React.FC = () => {
  ...
  const [decorationRequests, setDecorationRequests] = useState<DecorationRequest[]>([]);
...
useEffect(() => {
  ...
  const handleApproveRequest = async (request: PropertyRequest) => {
    try {
      const { approvePropertyRequest } = await import('./services/propertyRequestService');
  ...
  const handleEditAndApprove = async (editedPropertyData: any, originalRequestId: number) => {
    try {
      const { editAndPublishProperty } = await import('./services/propertyRequestService');
      await editAndPublishProperty(originalRequestId, editedPropertyData);
      setPropertyRequests(prev => prev.filter(r => r.id !== originalRequestId));
  ...
  const handleRejectRequest = async (requestId: number) => {
      try {
        const { rejectPropertyRequest } = await import('./services/propertyRequestService');
...
const hasPermission = (page: Page): boolean => {
  ...
  const renderPage = () => {
    ...
    switch (activePage) {
      ...
      default:
        return <DashboardPage setActivePage={setActivePage} properties={[]} inquiries={[]} propertyRequests={[]} finishingRequests={[]} decorationRequests={[]} portfolioItems={[]} refreshData={loadData} />;
    }
  };
