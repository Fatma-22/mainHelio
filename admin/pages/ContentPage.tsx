...
const ListingStatusBadge: React.FC<{ isListed: boolean; endDate: string }> = ({
  isListed,
  endDate,
}) => {
...
const PropertiesPage: React.FC<PropertiesPageProps> = ({
  ...
  const handleSaveProperty = (
    data: Omit<Property, "id" | "imageUrl" | "addedDate"> & { id?: number }
  ) => {
    if (data.id) {
      // Editing existing property
      setProperties(
        properties.map((p) => (p.id === data.id ? { ...p, ...data } : p))
      );
    } else {
      // Adding new property
