...
const PortfolioItemModal: React.FC<{
   isOpen: boolean;
  onClose: () => void;
  onSave: (item: Omit<PortfolioItem, 'id'> & { id?: number }, imageFile?: File | null, imageData?: { altText?: string; caption?: string }) => void;
  ...
  try {
    ...
    // Pass both the file and the metadata
    onSave(data, imageFile, imageData);
  } catch (error) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
      <div className="bg-gray-800 rounded-xl shadow-lg p-8 w-full max-w-2xl text-white max-h-[90vh] overflow-y-auto">
        <h2 className="text-2xl font-bold mb-6">{item ? 'تعديل العمل' : 'إضافة عمل جديد'}</h2>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <select 
              value={formData.type} 
              onChange={e => setFormData({...formData, type: e.target.value as DecorationRequestType})} 
...
const PortfolioPage: React.FC<PortfolioPageProps> = ({ 
  ...
  const [activeTab, setActiveTab] = useState<DecorationRequestType>('منحوتات جدارية');
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredItems.map(item => (
          <div key={item.id} className="bg-gray-800 rounded-xl shadow-lg overflow-hidden flex flex-col group transition-transform duration-300 hover:-translate-y-1">
            <img src={item.imageUrl} alt={item.title} className="w-full h-48 object-cover" loading='lazy' />
            <div className="p-4 flex flex-col flex-grow">
              <h3 className="text-lg font-bold text-white">{item.title}</h3>
              <p className="text-sm font-semibold text-blue-400 my-1">{item.type}</p>
