...
import type { DecorationRequest, DecorationRequestStatus, PortfolioItem } from '../types';
...
const DecorRequestsPage: React.FC<DecorationsPageProps> = ({ showToast }) => {
  ...
  const [selectedRequest, setSelectedRequest] = useState<DecorationRequest | null>(null);
  const [filters, setFilters] = useState({ status: 'الكل' });
  const [noteInput, setNoteInput] = useState('');
  const [selectedStatus, setSelectedStatus] = useState<DecorationRequestStatus>('جديد');
        ...
        {selectedRequest ? (
          <>
            ...
            <div className="p-6 overflow-y-auto flex-1 flex flex-col">
              {/* 🔹 قسم عرض الصورة */}
              <div className="mb-6">
                <h4 className="text-md font-bold text-white mb-2">صورة الطلب</h4>
                <div className="bg-gray-900/50 p-3 rounded-lg border border-gray-700 flex justify-center">
                  {getImageSource(selectedRequest) ? (
                    <div className="relative group cursor-pointer" onClick={() => openEnlargedImage(getImageSource(selectedRequest)!)}>
                      <img 
              ...
              <div className="flex justify-between items-center mt-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <select
                    ...
                    onChange={(e) => setSelectedStatus(e.target.value as DecorationRequestStatus)}
