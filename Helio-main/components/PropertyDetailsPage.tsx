...
const PropertyDetailsPage: React.FC<PropertyDetailsPageProps> = ({ language }) => {
    ...
    if (isLoading) {
        return (
             <div className="col-span-full text-center py-16">
                    <p className="text-xl text-gray-400">Loading Property Details</p>
                </div>
        )
    }
    ...
    const isForSale = property.status === 'For Sale' || property.status === 'للبيع'; // تم التعديل هنا
        ...
        <div className="bg-gray-900 text-white py-12">
            <div className="container mx-auto px-6">
                ...
                <div className="mb-8 flex flex-col sm:flex-row justify-between items-start gap-4">
                    <div>
                        <h1 className="text-4xl md:text-5xl font-bold">{property.title}</h1> {/* تم التعديل هنا */}
                        <p className="text-lg text-gray-400 mt-2">{property.address}</p> {/* تم التعديل هنا */}
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2">
                        ...
                        <div className="mb-8">
                            <div className="relative mb-4">
                               <img src={mainImage} alt={property.title} className="w-full h-[500px] object-cover rounded-lg shadow-lg" loading="lazy"  /> {/* تم التعديل هنا */}
                                <span className={`absolute top-4 ${language === 'ar' ? 'right-4' : 'left-4'} text-white font-semibold px-4 py-2 rounded-md text-base ${isForSale ? 'bg-green-600' : 'bg-sky-600'}`}>
                                  {property.status === 'For Sale' || property.status === 'للبيع' ? 
                                    ...
                                    (language === 'ar' ? 'للإيجار' : 'For Rent')} {/* تم التعديل هنا */}
                                </span>
                            </div>
                        ...
                        <div className="bg-gray-800 p-8 rounded-lg border border-gray-700 mb-8">
                            ...
                            <p className="text-gray-300 leading-relaxed whitespace-pre-line">{property.description || t.propertyDetailsPage?.noDescription}</p> {/* تم التعديل هنا */}
                        </div>
                    ...
                    <div className="lg:col-span-1">
                        <div className="sticky top-28 bg-gray-800 p-8 rounded-lg border border-gray-700">
                            <p className="text-4xl font-bold text-amber-500 mb-6">{property.price}</p> {/* تم التعديل هنا */}
                            ...
                            <div className="flex justify-around items-center text-gray-300 border-y border-gray-700 py-6 mb-6">
                                <div className="flex flex-col items-center gap-2">
                                    ...
                                    <span className="font-bold">{property.bedrooms}</span> {/* تم التعديل هنا */}
                                    <span>{t.propertyDetailsPage?.bedrooms}</span>
                                </div>
                               {isForSale && 
                                    (property.type === 'apartment' || property.type === 'villa' || property.type === 'شقة' || property.type === 'فيلا') && 
                                    property.finish && (
                                        <div className="flex flex-col items-center gap-2">
                                            <span className="font-bold">{property.finish}</span>
                                            <span>{t.propertyDetailsPage?.finishing || (language === 'ar' ? 'التشطيب' : 'Finishing')}</span>
                                        </div>
                                    )}

                                <div className="flex flex-col items-center gap-2">
                                    <AreaIcon className="w-8 h-8 text-gray-400" />
                                    <span className="font-bold">{property.area} m²</span> {/* تم التعديل هنا */}
