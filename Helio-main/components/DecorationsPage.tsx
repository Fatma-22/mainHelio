...
const DecorationsPage: React.FC<{ language: Language }> = ({ language }) => {
  ...
  const [activeTab, setActiveTab] = useState<'wall' | 'painting' | 'antique'>('wall');
  ...
  const [modalInfo, setModalInfo] = useState({
    ...
    reference_item_id: undefined as number | undefined,
  });
  ...
  const tabs = [
    { key: 'wall', name: t.tab1, desc: t.tab1Desc, serviceType: 'wall-decor', type: 'منحوتات جدارية' },
    { key: 'painting', name: t.tab2, desc: t.tab2Desc, serviceType: 'painting', type: 'لوحات كانفس' },
    { key: 'antique', name: t.tab3, desc: t.tab3Desc, serviceType: 'antique', type: 'تحف ديكورية' },
  ];

  const activeTabData = tabs.find(tab => tab.key === activeTab);
  
  // 🔹 تعديل: فلترة العناصر بناءً على النوع مع مراعاة التوافق مع الباك إند
  const filteredItems = portfolioItems.filter(item => item.type === activeTabData?.type);
    ...
    <div className="bg-gray-900 text-white">
      ...
      {modalInfo.isOpen && (
        <DecorationRequestModal
          ...
          reference_item_id={modalInfo.reference_item_id}
          language={language}
        />
      )}
      ...
      <div className="py-20">
        <div className="container mx-auto px-6">
          ...
          <div className="border-b border-gray-700 mb-12">
            <nav className="-mb-px flex justify-center space-x-4 lg:space-x-8">
              {tabs.map(tab => (
                <button
                  key={tab.key}
                    ...
                    <div className="flex flex-wrap justify-center gap-6 mb-12">
                      {filteredItems.map(item => (
                        ...
                        >
                          ...
                          <img
                            src={item.cover_url || item.imageUrl || '/placeholder-image.jpg'}
                            alt={item.title || (language === 'ar' ? 'صورة بورتفوليو' : 'Portfolio Image')}
                            className="w-full h-full object-cover transform transition-transform duration-300 group-hover:scale-105 cursor-pointer"
                            onClick={() => openLightbox(item.cover_url || item.imageUrl || '/placeholder-image.jpg')}
                            loading="lazy" 
                          ...
                          <div className="absolute inset-0 flex items-center justify-center p-2 pointer-events-none">
                            <button
                              onClick={(e) => {
                                ...
                                openModal(
                                  activeTabData.serviceType, 
                                  'similar', 
                                  item.cover_url || item.imageUrl, 
                                  item.id
                                );
                              }}
