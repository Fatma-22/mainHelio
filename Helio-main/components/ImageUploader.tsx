...
const SortableImageItem = ({ 
  ...
  isFeatured: boolean;
  onEditMetadata: (id: string, metadata: { altText: string; caption: string; seoKeywords: string }) => void;
}) => {
  const {
    attributes,
    listeners,
  ...
  return (
    <div
      ...
      className={`border rounded-lg overflow-hidden bg-gray-800 transition-all ${
        isFeatured ? 'border-amber-500 ring-2 ring-amber-500/50' : 'border-gray-700'
      }`}
    >
            ...
            <>
              ...
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-3.172l-1.414-1.414z" />
                </svg>
              </button>
              <button
                type="button"
      ...
      <div className="p-3">
          ...
          <div className="mt-2 space-y-1">
            {image.altText && (
              <div className="text-xs text-gray-400">
                <span className="font-medium">النص البديل:</span> {image.altText}
              </div>
            )}
            {image.caption && (
              <div className="text-xs text-gray-400">
            ...
            {image.seoKeywords && (
              <div className="text-xs text-gray-400">
                <span className="font-medium">الكلمات المفتاحية:</span> {image.seoKeywords}
              </div>
            )}
          </div>
        )}
        ...
        {isEditing && (
          <div className="mt-2 space-y-2">
            <div>
              <label className="block text-xs text-gray-400 mb-1">النص البديل (Alt Text)</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-xs text-gray-400 mb-1">العنوان (Caption)</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-xs text-gray-400 mb-1">الكلمات المفتاحية (SEO)</label>
              <input
                type="text"
...
const ImageUploader: React.FC<ImageUploaderProps> = ({
  ...
  const validateImage = (file: File): Promise<{ valid: boolean; error?: string; width?: number; height?: number }> => {
    return new Promise((resolve) => {
      ...
      if (!validTypes.includes(file.type)) {
        resolve({ valid: false, error: "نوع الملف غير مدعوم. يرجى استخدام JPEG, PNG, أو WebP." });
        return;
      }
      ...
      if (file.size > maxSize) {
        resolve({ valid: false, error: "حجم الملف كبير جدًا. الحد الأقصى هو 5 ميجابايت." });
        return;
      }
      ...
      img.onload = () => {
        ...
        if (width < 1200 || height < 800) {
          resolve({ valid: false, error: "أبعاد الصورة صغيرة جدًا. الحد الأدنى هو 1200x800 بكسل." });
          return;
        }
      ...
      try {
        ...
        if (validation.valid && validation.width && validation.height) {
          ...
          const validatedImage: ImageItem = {
            ...
            valid: true,
            originalDimensions: { // Store original dimensions
              width: validation.width,
              height: validation.height
            },
  ...
  const setFeaturedImage = (id: string) => {
    ...
    const isCurrentlyFeatured = images.some(img => img.id === id && img.isFeatured);
  ...
  // Edit image metadata
  const editImageMetadata = (id: string, metadata: { altText: string; caption: string; seoKeywords: string }) => {
    const updatedImages = images.map(image => {
      if (image.id === id) {
        return {
          ...image,
  ...
  const uploadImages = async () => {
    ...
    for (let i = 0; i < updatedImages.length; i++) {
      ...
      image.serverUrl = `https://example.com/uploads/${image.filename || image.id}`;
      image.thumbnailUrl = `https://example.com/uploads/thumbnails/${image.filename || image.id}`;
      image.mediumUrl = `https://example.com/uploads/medium/${image.filename || image.id}`;
    ...
    <div className="space-y-4">
      ...
      <div
        className={`border-2 border-dashed rounded-lg p-8 text-center cursor-pointer transition-colors ${
          isDragging ? 'border-amber-500 bg-amber-500/10' : 'border-gray-600 hover:border-amber-500'
        ...
        <div className="flex flex-col items-center justify-center space-y-2">
          {processing || isUploading ? (
            ...
            <>
              ...
              <p className="text-lg font-medium text-gray-300">اسحب وأفلت الصور هنا</p>
        ...
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                {selectedImages.length === images.length ? "إلغاء تحديد الكل" : "تحديد الكل"}
              </button>
          ...
          <div className="text-sm text-gray-400 bg-gray-800/50 p-3 rounded-lg">
            <div className="flex items-center mb-1">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 ml-2 text-amber-500" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
              </svg>
              <span>انقر على أيقونة النجمة لتحديد صورة مميزة (صورة واحدة فقط)</span>
            </div>
            <div className="flex items-center">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 ml-2 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="flex items-center">
              ...
              <span>انقر على أيقونة التعديل لتغيير النص البديل والعنوان والكلمات المفتاحية</span>
            </div>
          </div>
