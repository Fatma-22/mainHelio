...
const defaultPosition: [number, number] = [30.13831814569454, 31.679246011295543]; // وسط المدينة الجديد
...
const PropertiesMapPage: React.FC<PropertiesMapPageProps> = ({ language }) => {
    ...
    useEffect(() => {
        const fetchProperties = async () => {
            ...
            // لو في عقارات وواحد على الأقل فيه إحداثيات، خلي الخريطة مركزة عليه
        ...
        <div className="bg-gray-900">
            ...
            <div className="h-[75vh] w-full">
                <MapContainer center={mapCenter} zoom={14} scrollWheelZoom={true} style={{ height: '100%', width: '100%' }}>
                    ...
                    />
                    {properties.map(prop => {
                        // استخدم الإحداثيات الخاصة بالعقار أو الافتراضية
                        const lat = prop.latitude || defaultPosition[0];
                        const lng = prop.longitude || defaultPosition[1];
                        
                        return (
                            <Marker key={prop.id} position={[lat, lng]} icon={amberIcon}>
                                <Popup>
                                   <div className="w-full">
                                        <img src={prop.imageUrl} alt={prop.title} className="w-full h-32 object-cover rounded-t-lg" />
                                        <div className="p-4">
                                            <h3 className="text-md font-bold text-amber-500 mb-1 truncate">{prop.title}</h3>
                                            <p className="text-sm font-semibold text-gray-200 mb-3">{prop.price}</p>
                                             <div className="flex justify-between items-center text-gray-400 text-xs border-t border-gray-600 pt-3">
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    <span>{prop.bedrooms} {card_t?.beds || (language === 'ar' ? 'غرف' : 'Beds')}</span>
                                                </div>
                                                <div className="flex items-center gap-1">
                                                    <BathIcon className="w-4 h-4 text-gray-500" />
                                                    <span>{prop.bathrooms} {card_t?.baths || (language === 'ar' ? 'حمامات' : 'Baths')}</span>
                                                </div>
                                                <div className="flex items-center gap-1">
                                                    <AreaIcon className="w-4 h-4 text-gray-500" />
                                                    <span>{prop.area} {card_t?.area || (language === 'ar' ? 'م²' : 'm²')}</span>
                                                </div>
                                            </div>
                                            <Link 
