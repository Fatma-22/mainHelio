...
const PropertiesPage: React.FC<PropertiesPageProps> = ({ onAddPropertyClick, language }) => {
  ...
  const handleTabChange = (tab: 'buy' | 'rent') => {
    ...
    if (tab === 'rent' || (tab === 'buy' && typeFilter !== 'apartment' && typeFilter !== 'villa')) {
  ...
  const handleFilterChange = (filterName: string, value: string) => {
    ...
    if (filterName === 'type') {
      if (activeTab === 'rent' || (value !== 'apartment' && value !== 'villa')) {
        newParams.delete('finish');
      }
    }
  ...
  const normalizePropertyType = (type: string): string => {
    ...
    for (const [normalizedType, variations] of Object.entries(typeMap)) {
      if (variations.some(v => type.toLowerCase().includes(v.toLowerCase()))) {
  ...
  const filteredProperties = useMemo(() => {
    return properties.filter(p => {
      ...
      const statusMatch = activeTab === 'buy' ? 
        (p.status === 'For Sale' || p.status === 'للبيع') :
        (p.status === 'For Rent' || p.status === 'للإيجار');
      ...
      const priceMatch = priceNumeric >= priceRange.min && priceNumeric <= priceRange.max;
      
      const finishMatch = finishFilter === 'all' || normalizeFinish(p.finish) === finishFilter;
      ...
      const queryMatch = !queryFilter || 
        ...
        (p.description && p.description.toLowerCase().includes(queryFilter.toLowerCase()));
      return statusMatch && typeMatch && priceMatch && finishMatch && queryMatch;
    });
  }, [properties, activeTab, typeFilter, queryFilter, finishFilter, priceRange]);
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="mb-12 p-6 bg-gray-800 rounded-lg border border-gray-700">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {activeTab === 'buy' && (typeFilter === 'apartment' || typeFilter === 'villa') && (
              <select value={finishFilter} onChange={e => handleFilterChange('finish', e.target.value)} className={selectClasses}>
          ...
          <div className="mt-6 bg-gray-700/50 p-4 rounded-lg border border-gray-600">
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-xs text-gray-400">
                ...
                <span>{formatNumber(activeTab === 'buy' ? 90000000 : 90000)}</span>
              </div>
            </div>
          </div>
        </div>
